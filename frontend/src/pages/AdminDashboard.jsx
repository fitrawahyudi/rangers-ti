//  frontend/src/pages/AdminDashboard.jsx (Versi Bootstrap)
import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import api from "../api"; // Pastikan file api.js (axios instance) masih
...
export default function AdminDashboard() {
  ...
  // 1. State untuk melacak user mana yang sedang "dipilih"
  // Jika 'null', modal tertutup.
  // Jika berisi objek user, modal terbuka & menampilkan data user tsb.
  ...
  return (
    <div className="container my-4 md-my-5">
                ...
                {volunteers.length === 0 ? (
                  ...
                  volunteers.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td>{v.npm}</td>
                      <td>
                        <div className="fw-medium">{v.program_studi}</div>
                      </td>
                      <td>
                        <select
                          value={v.status}
      ...
      {/*
        ...
        - 'user={selectedUser}' -> Kirim data user yang dipilih ke modal
        - 'onClose={handleCloseModal}' -> Kirim fungsi untuk menutup modal
      */}
